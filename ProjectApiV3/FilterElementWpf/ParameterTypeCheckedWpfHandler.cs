...
{
    public class ParameterTypeCheckedWpfHandler : IExternalEventHandler
    {
        public void Execute(UIApplication app)
        {
                        ...
                        foreach (Parameter parae in element.Parameters)
                        {
                            if (parae.Id == pa.Id)
                            {
                                ...
                                if (!valueParameteres.Exists(x => x == value))
                                {
                                    ...
                                    listParameterValue.Add(paraValue);
            ...
            ObservableCollection<ParameterValue> observableCollection = new ObservableCollection<ParameterValue>();
            listParameterValue.ForEach(x => observableCollection.Add(x));
            AppPanelFilterWpf.myFormFilterElement.listViewValueParameter.ItemsSource = observableCollection;
            
        }

        public string GetName()
        {
            return "ParameterTypeCheckedWpf";
        }
    }
}
